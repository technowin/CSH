...
def export_to_excel(request):
    ...
    try:
        ...
        total_cell.value = f"Total Records: {total_count}"
        ...
        for row, app in enumerate(applications, data_start_row):
            for col, field in enumerate(fields, 1):
                ...
                if field == 'created_by' or field == 'updated_by':
                    if value:
                        try:
                            user_id_val = int(value)
                            value = user_names.get(user_id_val, str(value))
                        except (ValueError, TypeError):
        ...
        for col in range(1, len(fields) + 1):
            ...
            max_length = 0
            for row in range(header_row, min(ws.max_row + 1, header_row + 100)):
