...
def common_export_excel(request):
    ...
    try:
        ...
        # ========== ADD DATA ==========
        for row_idx, app_data in enumerate(all_applications, 2):
            for col_idx, col_name in enumerate(final_columns, 1):
        ...
        for col_idx, col_name in enumerate(final_columns, 1):
            ...
            for row_idx in range(2, min(len(all_applications) + 2, 100)):  # Check first 100 rows for performance
        ...
        # Summary header
        summary_ws.cell(row=1, column=1, value="Service-wise Summary").font = Font(bold=True, size=14)
